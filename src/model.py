...
class ChordRecognitionWithSmoothing(nn.Module):
    ...
    def forward(self, x, apply_smoothing=True):
        ...
        # During training: always return logits (for proper gradient flow)
        if self.training:
            return logits

        # During evaluation: return probabilities (optionally smoothed)
        ...
        if apply_smoothing and self.smoothing_window > 1:
            B, T, C = probs.shape
            kernel = (
                torch.ones(C, 1, 1, self.smoothing_window, device=probs.device)
